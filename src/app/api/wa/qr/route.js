...
export async function GET(request) {
  try {
    ...
    const externalUrl = `${APP_SERVICE}/api/wa/qr`;
    ...
    if (!externalRes.ok) {
      ...
      console.warn('Gagal fetch WA QR eksternal:', externalRes.status, errorText);
      return NextResponse.json({ error: 'Gagal mendapatkan QR dari server eksternal' }, { status: externalRes.status });
    }
    ...
    console.error('API /wa/qr error:', error);
