...
export async function GET(request) {
  try {
    ...
    const externalUrl = `${APP_SERVICE}/api/wa/status`;
    ...
    if (!externalRes.ok) {
      ...
      console.warn('Gagal fetch WA status eksternal:', externalRes.status, errorText);
      return NextResponse.json({ error: 'Gagal mendapatkan status dari server eksternal' }, { status: externalRes.status });
    }
    ...
    console.error('API /wa/status error:', error);
