...
export async function POST(request) {
  try {
    ...
    const externalUrl = `${APP_SERVICE}/api/wa/reconnect`;
    ...
    if (!externalRes.ok) {
      // Bisa jadi error atau 404/500
      const errorText = await externalRes.text();
      console.warn('Gagal fetch WA reconnect eksternal:', externalRes.status, errorText);
      return NextResponse.json({ error: 'Gagal melakukan reconnect ke server eksternal' }, { status: externalRes.status });
    }
    ...
    console.error('API /wa/reconnect error:', error);
