...
export async function GET(request) {
  try {
    // 1. Ambil query parameter 'bulan'
    const { searchParams } = new URL(request.url);
    const bulan = searchParams.get('bulan'); // Format: YYYY-MM

    if (!bulan) {
      return NextResponse.json({ error: 'Parameter bulan diperlukan (YYYY-MM)' }, { status: 400 });
    }

    // 2. Verifikasi Session Internal
    ...
    // 3. Ambil data user beserta token dan nomor WA
    ...
    const externalUrl = `${APP_SERVICE}/api/laporan/${phone}/chart/bulanan?bulan=${bulan}`;
    ...
    if (!externalRes.ok) {
      ...
      console.warn('Gagal fetch laporan bulanan eksternal:', externalRes.status, errorText);
    ...
    console.error('API /dashboard/laporan/bulanan error:', error);
